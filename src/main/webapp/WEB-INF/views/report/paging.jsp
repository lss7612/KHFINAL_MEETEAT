...
<div class="paging text-center">
	<ul class="pagination">
		...
		<c:if test="${paging.curPage ne 1 }">
			<li><a href="/report/list"> <span>&larr;</span></a></li>
<%-- 			<li><a href="/report/list?search=${param.search }"> <span>&larr;</span></a></li> --%>
		</c:if>
		<%-- 이전 페이지 버튼 --%>
		<%-- 첫 페이지면 금지 표시 --%>
		<c:if test="${paging.curPage ne 1 }">
			<li><a href="/report/list?curPage=${paging.curPage-1 }" aria-label="Previous"> <span>&laquo;</span></a></li>
<%-- 			<li><a href="/report/list?curPage=${paging.curPage-1 }&search=${param.search }" aria-label="Previous"> <span>&laquo;</span></a></li> --%>
		...
		<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="page"
			...
			<c:if test="${paging.curPage eq page }">
				<li class="active"><a href="/report/list?curPage=${page }">${page }</a></li>
<%-- 				<li class="active"><a href="/report/list?curPage=${page }&search=${param.search }">${page }</a></li> --%>
			</c:if>
			
			<c:if test="${paging.curPage ne page }">
				<li><a href="/report/list?curPage=${page }">${page }</a></li>
<%-- 				<li><a href="/report/list?curPage=${page }&search=${param.search }">${page }</a></li> --%>
		...
		<c:if test="${paging.curPage ne paging.totalPage }">
			<li><a href="/report/list?curPage=${paging.curPage+1 }" aria-label="Next"> <span>&raquo;</span></a></li>
<%-- 			<li><a href="/report/list?curPage=${paging.curPage+1 }&search=${param.search }" aria-label="Next"> <span>&raquo;</span></a></li> --%>
		...
		<%-- 마지막 페이지가 아닐 때 버튼 노출 --%>
		<c:if test="${paging.curPage ne paging.totalPage }">
			<li><a href="/report/list?curPage=${paging.totalPage }"> <span>&rarr;</span></a></li>
