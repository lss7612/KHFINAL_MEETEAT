...
<div class="base_padding">
<form id="articleSubmitForm" action="/eventboard/manage/modify" method="POST">

	<%-- 글제목 --%>
	<div class="form-group">
		<label id="labebforform_article_title"for="form_article_title">제목</label>
		<input type="text" id="form_article_title" name="article_title" value="${result.ARTICLE_TITLE }" class="form-control" alertComment="글제목"/>
	</div>
	<hr>
	...
	<div class="pull-left form-group" style="min-width:155px; width:20%; margin:0 30px 0 0;">
		...
		<input type="date" id="form_start_date" name="start_date" value="${result.START_DATE }" class="form-control" alertComment="시작일"/>
	</div>
	
	<%-- 종료일 --%>
	<div class="pull-left form-group" style="min-width:155px; width:20%; margin:0 30px 0 0;">
		<label for="form_terminate_date">종료일</label>
		<input type="date" id="form_terminate_date" name="terminate_date" value="${result.TERMINATE_DATE }" class="form-control" alertComment="종료일"/>
	</div>
	...
	<c:otherwise>
		<div style="width:510px; margin: 0 auto;" >
			...
			<div style="width:100%; margin: 0 auto;" >
				<%--이미지박스1 아래의 이미지폼과 스크립트 코드로 연결 --%>
				<div id="bigImgBox"class="light_margin bigImgBox none-image pull-left">
				<img class="big-upload-image" alt="" src="/resources/file/recruitboard/save/${result.IMGLIST.get(0).FILE_STOREDNAME }">
				</div>
			</div>
	...
	<div class=" form-group small_form invisible none_height">
		<%-- board_no submit --%>
		<label for="form_board_no" >ext01</label>
		<input type="text" id="form_board_no" name="board_no" value="6"class="form-control"/>
	</div>
	<div class=" form-group small_form invisible none_height">
		<%-- board_no submit --%>
		<label for="form_article_no" >ext01</label>
		<input type="text" id="form_article_no" name="article_no" value="${result.ARTICLE_NO }"class="form-control"/>
	</div>
	
	<%-- 내용 --%>
	<div class="form-group" style="min-width:550px;"> 
		<label for="form_article_content">내용</label>
		<textarea rows="20" id="form_article_content" name="article_content" style="min-width:550px; width:100%;" alertComment="내용">${result.ARTICLE_CONTENT }</textarea>
	</div>
	
	<div class="base_bar_height light_padding"><button id="btn_submit" type="button" class="pull-right btn btn-primary">수정하기</button></div>
