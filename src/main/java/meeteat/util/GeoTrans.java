 ...
 * @author aquilegia
 *
 *         The code based on hyosang(http://hyosang.kr/tc/96) and aero's blog
 *         ((http://aero.sarang.net/map/analysis.html) License: LGPL :
...
public class GeoTrans {
	...
	static {
		...
		src_m[GEO] = m_arMajor[GEO]
				* mlfn(e0fn(m_Es[GEO]), e1fn(m_Es[GEO]), e2fn(m_Es[GEO]), e3fn(m_Es[GEO]), m_arLatCenter[GEO]);
		dst_m[GEO] = m_arMajor[GEO]
				* mlfn(e0fn(m_Es[GEO]), e1fn(m_Es[GEO]), e2fn(m_Es[GEO]), e3fn(m_Es[GEO]), m_arLatCenter[GEO]);
		src_m[KATEC] = m_arMajor[KATEC] * mlfn(e0fn(m_Es[KATEC]), e1fn(m_Es[KATEC]), e2fn(m_Es[KATEC]),
				e3fn(m_Es[KATEC]), m_arLatCenter[KATEC]);
		dst_m[KATEC] = m_arMajor[KATEC] * mlfn(e0fn(m_Es[KATEC]), e1fn(m_Es[KATEC]), e2fn(m_Es[KATEC]),
				e3fn(m_Es[KATEC]), m_arLatCenter[KATEC]);
		src_m[TM] = m_arMajor[TM]
				* mlfn(e0fn(m_Es[TM]), e1fn(m_Es[TM]), e2fn(m_Es[TM]), e3fn(m_Es[TM]), m_arLatCenter[TM]);
		dst_m[TM] = m_arMajor[TM]
				* mlfn(e0fn(m_Es[TM]), e1fn(m_Es[TM]), e2fn(m_Es[TM]), e3fn(m_Es[TM]), m_arLatCenter[TM]);
	}
	...
	public static void geo2tm(int dsttype, GeoTransPoint in_pt, GeoTransPoint out_pt) {
		...
		double ml = m_arMajor[dsttype]
				* mlfn(e0fn(m_Es[dsttype]), e1fn(m_Es[dsttype]), e2fn(m_Es[dsttype]), e3fn(m_Es[dsttype]), in_pt.y);

		out_pt.x = m_arScaleFactor[dsttype] * n * al
				* (1.0 + als / 6.0
						* (1.0 - t + c + als / 20.0 * (5.0 - 18.0 * t + t * t + 72.0 * c - 58.0 * m_Esp[dsttype])))
				+ m_arFalseEasting[dsttype];
		out_pt.y = m_arScaleFactor[dsttype]
				* (ml - dst_m[dsttype] + n * tq * (als * (0.5 + als / 24.0
						* (5.0 - t + 9.0 * c + 4.0 * c * c
								+ als / 30.0 * (61.0 - 58.0 * t + t * t + 600.0 * c - 330.0 * m_Esp[dsttype])))))
				+ m_arFalseNorthing[dsttype];
	}
	...
	public static void tm2geo(int srctype, GeoTransPoint in_pt, GeoTransPoint out_pt) {
		...
		while (true) {
			double delta_Phi = ((con + e1fn(m_Es[srctype]) * Math.sin(2.0 * phi)
					- e2fn(m_Es[srctype]) * Math.sin(4.0 * phi) + e3fn(m_Es[srctype]) * Math.sin(6.0 * phi))
					/ e0fn(m_Es[srctype])) - phi;
			phi = phi + delta_Phi;
		...
		if (Math.abs(phi) < (Math.PI / 2)) {
			...
			double ds = d * d;
			out_pt.y = phi - (n * tan_phi * ds / r) * (0.5 - ds / 24.0 * (5.0 + 3.0 * t + 10.0 * c - 4.0 * cs
					- 9.0 * m_Esp[srctype]
					- ds / 30.0 * (61.0 + 90.0 * t + 298.0 * c + 45.0 * ts - 252.0 * m_Esp[srctype] - 3.0 * cs)));
			out_pt.x = m_arLonCenter[srctype] + (d
					* (1.0 - ds / 6.0
							* (1.0 + 2.0 * t + c
									- ds / 20.0
											* (5.0 - 2.0 * c + 28.0 * t - 3.0 * cs + 8.0 * m_Esp[srctype] + 24.0 * ts)))
					/ cos_phi);
		} else {
	...
	public static double getDistancebyGeo(GeoTransPoint pt1, GeoTransPoint pt2) {
		...
		double a = Math.pow(Math.sin(latitude / 2.0), 2)
	...
	/*
	 * Author: Richard Greenwood rich@greenwoodmap.com License: LGPL as per:
	 * http://www.gnu.org/copyleft/lesser.html
	 */

	/**
	 * convert between geodetic coordinates (longitude, latitude, height) and
	 * gecentric coordinates (X, Y, Z) ported from Proj 4.9.9 geocent.c
	 */
	...
	private static boolean geodetic_to_geocentric(int type, GeoTransPoint p) {
		 ...
		 * longitude in radians (input) Height : Geodetic height, in meters (input) X :
		 * Calculated Geocentric X coordinate, in meters (output) Y : Calculated
		 * Geocentric Y coordinate, in meters (output) Z : Calculated Geocentric Z
		 * coordinate, in meters (output)
		 *
		 */
		...
		/*
		 ** Don't blow up if Latitude is just a little out of the value range as it may
		 * just be a rounding issue. Also removed longitude test, it should be wrapped
		 * by Math.cos() and Math.sin(). NFW for PROJ.4, Sep/2001.
		 */
	...
	/**
	 * Convert_Geocentric_To_Geodetic The method used here is derived from 'An
	 * Improved Algorithm for Geocentric to Geodetic Coordinate Conversion', by
	 * Ralph Toms, Feb 1996
	 */
